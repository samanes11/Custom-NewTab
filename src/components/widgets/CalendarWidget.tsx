...
import { WidgetFrame, useWidgetSize } from "@/components/common/WidgetFrame";
...
function CompactStrip() {
    ...
    const days = Array.from({ length: 7 }, (_, i) => {
        ...
        return { jd: j.jd, isToday: offset === 0, key: `${j.jy}-${j.jm}-${j.jd}` };
    });

    return (
        <div dir="rtl" className="flex h-full items-center justify-between gap-1">
            {days.map((d) => (
                <span
                    key={d.key}
                    className={`tabular flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs ${d.isToday ? "bg-accent font-semibold text-white" : "text-ink-dim"
                        }`}
                >
                    {d.jd}
                </span>
...
function CalendarBody() {
    ...
    return height < 170 ? <CompactStrip /> : <FullMonth />;
}
