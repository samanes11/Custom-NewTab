...
import {
  ...
  StateView,
  useWidgetSize,
} from "@/components/common/WidgetFrame";
...
function GithubBody({
  ...
  return (
    <div className={stale ? "opacity-70" : ""}>
      <div
        className={`flex items-center gap-3 ${compact ? "" : "mb-4"
          }`}
      >
        <img
          src={profile.avatarUrl}
          alt=""
          className="h-10 w-10 shrink-0 rounded-full border border-surface-border"
        />
      ...
      {!compact &&
        (profile.hasFullData && profile.weeks ? (
          ...
          <p className="text-xs leading-relaxed text-ink-faint">
            Add a personal access token in Settings → GitHub to see your
            contribution calendar and streak.
          </p>
        ))}
    </div>
  );
...
export function GithubWidget({ settings, update }: Props) {
  ...
  const { state } = useAsyncResource<GithubProfile>(
    ...
    [username, token],
    {
      intervalMs: REFRESH_INTERVALS.github,
  ...
  return (
    ...
    >
      <StateView
        state={state}
        emptyLabel="Add your GitHub username in Settings"
        skeletonLines={4}
      >
        {(profile, stale) => (
          <GithubBody profile={profile} stale={stale} />
        )}
      </StateView>
    </WidgetFrame>
  );
