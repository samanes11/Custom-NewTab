...
function CurrencyBody({ quotes, stale }: { quotes: CurrencyQuote[]; stale?: boolean }) {
  const { width, height } = useWidgetSize();
  const narrow = width < 170;
  const short = height < 140;
  ...
  return (
    <WidgetFrame icon={DollarSign} title="Currency" settings={<CurrencySection settings={settings} update={update} />}>
      <StateView state={state} emptyLabel="No currencies configured" skeletonLines={4}>
        {(quotes, stale) => <CurrencyBody quotes={quotes} stale={stale} />}
      </StateView>
    </WidgetFrame>
  );
}